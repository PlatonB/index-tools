...
[yes(|y)|no(|n|<enter>)]: ''')
        ...
        #Размещение данных в базу наборами по 10000
        #строк, как показала практика, не может
        #осуществляться при дефолтных лимитах MySQL.
        #На время выполнения программы эти значения будут
        #приравнены к объёму RAM компьютера исследователя.
        cursor.execute(f'SET GLOBAL key_buffer_size={ram}*1024*1024*1024')
        cursor.execute(f'SET GLOBAL max_allowed_packet={ram}*1024*1024*1024')
